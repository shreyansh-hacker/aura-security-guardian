...
// Use only icons allowed from the allowed list:
  ...
  return (
    <div className="bg-white rounded-2xl shadow p-4">
      <h3 className="font-semibold text-lg mb-2 flex items-center gap-2">
        <Bell className="w-5 h-5 text-red-500" /> Real-time Alerts
      </h3>
      <div className="space-y-3 h-[340px] overflow-y-auto pr-2">
        {alerts.filter(Boolean).length === 0 && (
          <div className="text-gray-500">No alerts.</div>
        )}
        {alerts
          .filter(Boolean)
          .filter(alert => alert && alert.icon && alert.message && alert.time)
          .map((alert, i) => (
            <div
              key={i}
